        ...
        Ok(match node.kind() {
            ...
            _ => {
                return Err(ParseError::new(
                    node.range(),
                    format!(
                        "NamedTypeChildren: Parse error, unexpected node-type: {}",
                        node.kind()
                    ),
                ))
            }
...
impl NodeParser for NamedTypeNode {
    fn parse(node: Node, source: &[u8]) -> Result<Self, ParseError> {
        ...
        if node.kind() != "named_type" {
            return Err(ParseError::new(range, format!("NamedTypeNode: Node is of the wrong kind [{}] vs expected [named_type] on pos {}:{}", node.kind(), range.start_point.row+1, range.start_point.column)));
        }
