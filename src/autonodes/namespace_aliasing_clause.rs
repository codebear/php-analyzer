...
impl NodeParser for NamespaceAliasingClauseNode {
    fn parse(node: Node, source: &[u8]) -> Result<Self, ParseError> {
        ...
        if node.kind() != "namespace_aliasing_clause" {
            return Err(ParseError::new(range, format!("NamespaceAliasingClauseNode: Node is of the wrong kind [{}] vs expected [namespace_aliasing_clause] on pos {}:{}", node.kind(), range.start_point.row+1, range.start_point.column)));
        }
