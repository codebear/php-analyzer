        ...
        Ok(match node.kind() {
            ...
            _ => {
                return Err(ParseError::new(
                    node.range(),
                    format!(
                        "SubscriptExpressionDereferenceable: Parse error, unexpected node-type: {}",
                        node.kind()
                    ),
                ))
            }
...
impl NodeParser for SubscriptExpressionNode {
    fn parse(node: Node, source: &[u8]) -> Result<Self, ParseError> {
        ...
        if node.kind() != "subscript_expression" {
            return Err(ParseError::new(range, format!("SubscriptExpressionNode: Node is of the wrong kind [{}] vs expected [subscript_expression] on pos {}:{}", node.kind(), range.start_point.row+1, range.start_point.column)));
        }
