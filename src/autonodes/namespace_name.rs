...
impl NodeParser for NamespaceNameNode {
    fn parse(node: Node, source: &[u8]) -> Result<Self, ParseError> {
        ...
        if node.kind() != "namespace_name" {
            return Err(ParseError::new(range, format!("NamespaceNameNode: Node is of the wrong kind [{}] vs expected [namespace_name] on pos {}:{}", node.kind(), range.start_point.row+1, range.start_point.column)));
        }
