            ...
            _ => {
                    ...
                    .map(DeclareStatementChildren::_Statement)
                {
                    ...
                    return Err(ParseError::new(
                        node.range(),
                        format!(
                            "DeclareStatementChildren: Parse error, unexpected node-type: {}",
                            node.kind()
                        ),
                    ));
                }
...
impl NodeParser for DeclareStatementNode {
    fn parse(node: Node, source: &[u8]) -> Result<Self, ParseError> {
        ...
        if node.kind() != "declare_statement" {
            return Err(ParseError::new(range, format!("DeclareStatementNode: Node is of the wrong kind [{}] vs expected [declare_statement] on pos {}:{}", node.kind(), range.start_point.row+1, range.start_point.column)));
        }
