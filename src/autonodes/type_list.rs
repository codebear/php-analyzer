...
impl NodeParser for TypeListNode {
    fn parse(node: Node, source: &[u8]) -> Result<Self, ParseError> {
        ...
        if node.kind() != "type_list" {
            return Err(ParseError::new(range, format!("TypeListNode: Node is of the wrong kind [{}] vs expected [type_list] on pos {}:{}", node.kind(), range.start_point.row+1, range.start_point.column)));
        }
