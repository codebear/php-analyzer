        ...
        Ok(match node.kind() {
            ...
            _ => {
                return Err(ParseError::new(
                    node.range(),
                    format!(
                        "SwitchBlockChildren: Parse error, unexpected node-type: {}",
                        node.kind()
                    ),
                ))
            }
...
impl NodeParser for SwitchBlockNode {
    fn parse(node: Node, source: &[u8]) -> Result<Self, ParseError> {
        ...
        if node.kind() != "switch_block" {
            return Err(ParseError::new(range, format!("SwitchBlockNode: Node is of the wrong kind [{}] vs expected [switch_block] on pos {}:{}", node.kind(), range.start_point.row+1, range.start_point.column)));
        }
