...
impl NodeParser for FinallyClauseNode {
    fn parse(node: Node, source: &[u8]) -> Result<Self, ParseError> {
        ...
        if node.kind() != "finally_clause" {
            return Err(ParseError::new(range, format!("FinallyClauseNode: Node is of the wrong kind [{}] vs expected [finally_clause] on pos {}:{}", node.kind(), range.start_point.row+1, range.start_point.column)));
        }
