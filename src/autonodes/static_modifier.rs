...
impl NodeParser for StaticModifierNode {
    fn parse(node: Node, source: &[u8]) -> Result<Self, ParseError> {
        ...
        if node.kind() != "static_modifier" {
            return Err(ParseError::new(range, format!("StaticModifierNode: Node is of the wrong kind [{}] vs expected [static_modifier] on pos {}:{}", node.kind(), range.start_point.row+1, range.start_point.column)));
        }
