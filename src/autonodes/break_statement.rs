...
impl NodeParser for BreakStatementNode {
    fn parse(node: Node, source: &[u8]) -> Result<Self, ParseError> {
        ...
        if node.kind() != "break_statement" {
            return Err(ParseError::new(range, format!("BreakStatementNode: Node is of the wrong kind [{}] vs expected [break_statement] on pos {}:{}", node.kind(), range.start_point.row+1, range.start_point.column)));
        }
