...
impl NodeParser for MatchConditionalExpressionNode {
    fn parse(node: Node, source: &[u8]) -> Result<Self, ParseError> {
        ...
        if node.kind() != "match_conditional_expression" {
            return Err(ParseError::new(range, format!("MatchConditionalExpressionNode: Node is of the wrong kind [{}] vs expected [match_conditional_expression] on pos {}:{}", node.kind(), range.start_point.row+1, range.start_point.column)));
        }
