...
impl NodeParser for GotoStatementNode {
    fn parse(node: Node, source: &[u8]) -> Result<Self, ParseError> {
        ...
        if node.kind() != "goto_statement" {
            return Err(ParseError::new(range, format!("GotoStatementNode: Node is of the wrong kind [{}] vs expected [goto_statement] on pos {}:{}", node.kind(), range.start_point.row+1, range.start_point.column)));
        }
