...
impl NodeParser for HeredocEndNode {
    fn parse(node: Node, source: &[u8]) -> Result<Self, ParseError> {
        ...
        if node.kind() != "heredoc_end" {
            return Err(ParseError::new(range, format!("HeredocEndNode: Node is of the wrong kind [{}] vs expected [heredoc_end] on pos {}:{}", node.kind(), range.start_point.row+1, range.start_point.column)));
        }
