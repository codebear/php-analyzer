...
impl NodeParser for NamespaceUseGroupNode {
    fn parse(node: Node, source: &[u8]) -> Result<Self, ParseError> {
        ...
        if node.kind() != "namespace_use_group" {
            return Err(ParseError::new(range, format!("NamespaceUseGroupNode: Node is of the wrong kind [{}] vs expected [namespace_use_group] on pos {}:{}", node.kind(), range.start_point.row+1, range.start_point.column)));
        }
