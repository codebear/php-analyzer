...
impl NodeParser for DefaultStatementNode {
    fn parse(node: Node, source: &[u8]) -> Result<Self, ParseError> {
        let range: Range = node.range().into();
        if node.kind() != "default_statement" {
            return Err(ParseError::new(range, format!("DefaultStatementNode: Node is of the wrong kind [{}] vs expected [default_statement] on pos {}:{}", node.kind(), range.start_point.row+1, range.start_point.column)));
        }
