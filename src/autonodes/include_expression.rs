...
impl NodeParser for IncludeExpressionNode {
    fn parse(node: Node, source: &[u8]) -> Result<Self, ParseError> {
        ...
        if node.kind() != "include_expression" {
            return Err(ParseError::new(range, format!("IncludeExpressionNode: Node is of the wrong kind [{}] vs expected [include_expression] on pos {}:{}", node.kind(), range.start_point.row+1, range.start_point.column)));
        }
