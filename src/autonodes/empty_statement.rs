...
impl NodeParser for EmptyStatementNode {
    fn parse(node: Node, source: &[u8]) -> Result<Self, ParseError> {
        ...
        if node.kind() != "empty_statement" {
            return Err(ParseError::new(range, format!("EmptyStatementNode: Node is of the wrong kind [{}] vs expected [empty_statement] on pos {}:{}", node.kind(), range.start_point.row+1, range.start_point.column)));
        }
