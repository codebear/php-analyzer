...
impl NodeParser for EscapeSequenceNode {
    fn parse(node: Node, source: &[u8]) -> Result<Self, ParseError> {
        ...
        if node.kind() != "escape_sequence" {
            return Err(ParseError::new(range, format!("EscapeSequenceNode: Node is of the wrong kind [{}] vs expected [escape_sequence] on pos {}:{}", node.kind(), range.start_point.row+1, range.start_point.column)));
        }
