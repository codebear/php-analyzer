...
impl NodeParser for CommentNode {
    fn parse(node: Node, source: &[u8]) -> Result<Self, ParseError> {
        ...
        if node.kind() != "comment" {
            return Err(ParseError::new(range, format!("CommentNode: Node is of the wrong kind [{}] vs expected [comment] on pos {}:{}", node.kind(), range.start_point.row+1, range.start_point.column)));
        }
