        ...
        Ok(match node.kind() {
            ...
            _ => {
                return Err(ParseError::new(
                    node.range(),
                    format!(
                        "AttributeChildren: Parse error, unexpected node-type: {}",
                        node.kind()
                    ),
                ))
            }
...
impl NodeParser for AttributeNode {
    fn parse(node: Node, source: &[u8]) -> Result<Self, ParseError> {
        ...
        if node.kind() != "attribute" {
            return Err(ParseError::new(range, format!("AttributeNode: Node is of the wrong kind [{}] vs expected [attribute] on pos {}:{}", node.kind(), range.start_point.row+1, range.start_point.column)));
        }
