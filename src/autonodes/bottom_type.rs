...
impl NodeParser for BottomTypeNode {
    fn parse(node: Node, source: &[u8]) -> Result<Self, ParseError> {
        ...
        if node.kind() != "bottom_type" {
            return Err(ParseError::new(range, format!("BottomTypeNode: Node is of the wrong kind [{}] vs expected [bottom_type] on pos {}:{}", node.kind(), range.start_point.row+1, range.start_point.column)));
        }
