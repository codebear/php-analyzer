...
impl NodeParser for ReadonlyModifierNode {
    fn parse(node: Node, source: &[u8]) -> Result<Self, ParseError> {
        ...
        if node.kind() != "readonly_modifier" {
            return Err(ParseError::new(range, format!("ReadonlyModifierNode: Node is of the wrong kind [{}] vs expected [readonly_modifier] on pos {}:{}", node.kind(), range.start_point.row+1, range.start_point.column)));
        }
