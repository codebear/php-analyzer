...
impl NodeParser for DeclareDirectiveNode {
    fn parse(node: Node, source: &[u8]) -> Result<Self, ParseError> {
        ...
        if node.kind() != "declare_directive" {
            return Err(ParseError::new(range, format!("DeclareDirectiveNode: Node is of the wrong kind [{}] vs expected [declare_directive] on pos {}:{}", node.kind(), range.start_point.row+1, range.start_point.column)));
        }
