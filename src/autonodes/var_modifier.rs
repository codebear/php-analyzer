...
impl NodeParser for VarModifierNode {
    fn parse(node: Node, source: &[u8]) -> Result<Self, ParseError> {
        let range: Range = node.range().into();
        if node.kind() != "var_modifier" {
            return Err(ParseError::new(range, format!("VarModifierNode: Node is of the wrong kind [{}] vs expected [var_modifier] on pos {}:{}", node.kind(), range.start_point.row+1, range.start_point.column)));
        }
