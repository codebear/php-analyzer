...
impl NodeParser for VisibilityModifierNode {
    fn parse(node: Node, source: &[u8]) -> Result<Self, ParseError> {
        ...
        if node.kind() != "visibility_modifier" {
            return Err(ParseError::new(range, format!("VisibilityModifierNode: Node is of the wrong kind [{}] vs expected [visibility_modifier] on pos {}:{}", node.kind(), range.start_point.row+1, range.start_point.column)));
        }
